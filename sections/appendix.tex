...
endpoint. It submits each of the $60$ targets under
...
(DSPv2-$7$B GPTQ-int8) were each extended on the full $60$-target
slate.

Across the $6 \times 60 = 360$ (drafter, target) cells, doubling
...
results in order to keep the main text within the page
...
\emph{Majority} targets exercise a rewrite step that most
...
per-attempt API cost aggregated across the $60$-target benchmark
...
$60$-target benchmark. Attempt counts include duplicate runs after a
...
per-drafter total API cost on the $60$-target benchmark divided by the
number of targets closed at pass@$5$. This avoids treating
attempts as independent Bernoulli trials, which they are not.
On the $60$-target benchmark at pass@$5$: Mistral closes $2/60$ at
$\$0.87$ total ($\$0.44$ per closed target on the neutral
prompt). Sonnet closes $18/60$ at $\$5.71$ total ($\$0.32$
per closed target). Kimi closes $18/60$ at $\$2.30$ total
($\$0.13$ per closed target). Gemini closes $22/60$ at
$\$14.67$ total ($\$0.67$ per closed target). Opus closes
$13/60$ at $\$13.75$ total ($\$1.06$ per closed target).
DSPv2 closes $12/60$ at $\$0$ marginal API cost. The ranking changes if measured
...
enumerated list ($0/60$ on hint-list). Without the enumeration,
pass@$5$ rises to $2/60$, recovering the T3 $\sqrt{2}$ identity
and one T1 target.
...
On our slate, DSPv2-7B~\cite{dspv22025} closes $9/60$ at pass@$5$
while Goedel-Prover-V2-8B~\cite{goedelprover2025} closes $5/60$
...
$5/60$ to $13/60$ ($2.6\times$). This surpasses DSPv2's single-shot
$9/60$, making Goedel-V2 the strongest local prover on our slate at
